...
class Client {
    ...
    _start() {
        ...
        function bootstrap(ch) {
            ...
            return ch.assertQueue('', options).then(startConsuming).then(assertConsumeOk);
        }
    ...
    _sendRequest(method, args, correlationId, replyTo) {
        ...
        this.channel.publish('', this.serviceName, utils.messageToBuffer(message), options);
    }
    ...
    _getNewCorrelationId() {
        ...
        if (requestNumber == Number.MAX_SAFE_INTEGER) {
            requestNumber = this.requestNumber = 0;
        }
    ...
    call(method) {
        ...
        this._sendRequest(method, args, slot.correlationId, this.responseQueue.name);
